...
function Login(props) {
  ...
  const handleSubmit = async e => {
    ...
    try {
      ...
      if (res.ok) {
        if (resData.user && resData.user.isAdmin) {
          // Admin login
          if (props.onAdminLogin) props.onAdminLogin(resData.user);
          setForm({ name: "", password: "" });
          setMsg("");
          navigate("/admin"); // Redirect to Admin page route
        } else {
          // Normal user login
          if (props.onLoginSuccess) props.onLoginSuccess(resData.user);
          setForm({ name: "", password: "" });
          setMsg("");
          navigate("/home"); // Redirect to user home or dashboard
        }
      } else {
